...
export default {
  ...
  methods: {
    ...
    async getemails() {
      try {
        const response = await axios.get(
          `https://email-server-back.onrender.com/userfolders/getuserfoldersbyid/${this.$route.query.userid}/${this.id}`
        );
        this.emails = response.data;
    ...
    move(id) {
      ...
      this.$router.push({
        name: "movemodal", // <-- Make sure this matches the route name
        query: { folder: this.id, myList: q, userid: this.Userid },
      });
    },
    async deleteEmails() {
      for (let i = 0; i < this.emails["length"]; i++) {
    ...
    openMail(id) {
      this.$router.push({
        path: `/UserFolders/${this.id}/mail`,
        query: {
          userid: this.$route.query.userid,
          id: id,
          path: `${"getmail/" + `${this.id}`}`,
        },
      });
    },
  },
