<template>
  <div class="draft">
    ...
    <div class="sort">
      <button class="sort-button" @click="openModal2">
        sort
      </button>
    </div>
    ...
    <div class="actions">
      ...
      <!-- <span class="fa fa-envelope" @click="markAsUnread(mail.id)"></span> -->
...
export default {
  ...
  mounted() {
    fetch(`https://email-server-back.onrender.com/mainfolders/getdraft/${this.userId}`)
      .then((res) => res.json())
  ...
  methods: {
    ...
    async sortAction() {
      ...
      const response = await axios.get(
        `https://email-server-back.onrender.com/sort/getsorted/${this.$route.query.userid}/${this.sortway}/draft`
      );
    ...
    openComposer(id) {
      this.$router.push(`/newEmail/${id}?userid=${this.userId}`);
    },
  },
};
</script>
...
.actions span {
  margin-right: 10px;
  cursor: pointer;
  color: #007bff; /* Set the color as per your design */
}
