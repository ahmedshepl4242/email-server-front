  ...
  <div class="trash">
    ...
    <div v-for="(mail, index) in emails" :key="mail.id">
      ...
      <div class="actions">
        ...
        <span class="fa fa-mail-reply" @click="Restore(mail.id)"></span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getemails() {
      try {
        const response = await axios.get(
          `https://email-server-back.onrender.com/mainfolders/gettrash/${this.$route.query.userid}`
        );
        this.emails = response.data;
        this.selectedEmails = [];
        ...
        console.log("inbox emails retrieved successfully!");
      } catch (error) {
    ...
    async Restore(data) {
      const response = await axios.put(
        `https://email-server-back.onrender.com/mails/restoretrash/${this.$route.query.userid}/${data}`
      );
    },
  },
};
